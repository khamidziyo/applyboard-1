...
if (!empty($_POST)) {
    try {
        ...
        $agent_exist = $wpdb->get_results("select id,email,password,status,role from agents 
        where email='" . $email . "' && role='3'");

        if (empty($agent_exist[0])) {
            throw new Exception("This email address does not exist");
        }

        $hash_pass = $agent_exist[0]->password;
        ...
        $payload = [
            ...
            'iss' => 'localhost',
            'exp' => time() + (24 * 60 * 60),
            'userId' => $agent_exist[0]->id,
        ];
        $access_token = Jwt::encode($payload, Secret_Key);
        
        $data=['email'=>$agent_exist[0]->email,'role'=>$agent_exist[0]->role,'token'=>$access_token];
