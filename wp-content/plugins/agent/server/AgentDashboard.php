    ...
    if (verifyUser()) {
        switch ($_POST['val']) {
            ...
            case 'adminDashboard':
                ...
                $total_application = $wpdb->get_results("select count(id) as total_application from applications where user_id=" . $id);
                $applications = $total_application[0]->total_application;

                $app_application = $wpdb->get_results("select count(id) as approve_application from applications where status='1' && user_id=" . $id);
                $approved_application = $app_application[0]->approve_application;

                $dec_application = $wpdb->get_results("select count(id) as decline_application from applications where status='2' && user_id=" . $id);
                $declined_application = $dec_application[0]->decline_application;

                $pen_application = $wpdb->get_results("select count(id) as pending_application from applications where status='0' && user_id=" . $id);
